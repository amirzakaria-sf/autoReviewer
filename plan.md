# WhipGuard — end-to-end architecture and build plan
...
**One sentence.** WhipGuard watches a connected repository, finds real problems across five
...
1. Bug Council (per enabled category)
     ...
     score >= threshold?
        yes -> GitHub issue raised, labeled `whipguard:bug`, `whipguard:category/<key>`,
               `whipguard:severity/<n>` ; dashboard row created ; Slack + email notified
...
2. Fix Council (triggered by a raised issue, runs automatically — proposing costs nothing
   the user has to approve; only ACTING does)
     ...
     score >= threshold?
        yes -> GitHub draft PR opened, labeled `whipguard:fix-proposed`,
               `whipguard:awaiting-approval` ; dashboard row updated with full diff and
...
4. On approval:
     ...
     post-deploy oracle: the SAME Playwright check that ran pre-approval, re-run against
       ...
       PR `whipguard:verification-failed` and notifies rather than silently claiming success
     on success: `whipguard:deployed` -> `whipguard:verified` ; final notification with the
...
- **Notifications go quiet for a short window around a deploy of WhipGuard's own
...
agent's instructions forever. WhipGuard ships that test from day one for every stable
...
WhipGuard wires up (Playwright, the GitHub API, Cloudflare, Slack, email): a new integration
